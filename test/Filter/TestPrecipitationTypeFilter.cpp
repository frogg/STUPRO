...
void testDataSet(vtkSmartPointer<vtkUnstructuredGrid> dataSet, int expectedPointAmount, QList<PrecipitationDataPoint::PrecipitationType> disallowedPrecipitationTypes) {
	...
	vtkSmartPointer<vtkTypeInt32Array> precipitationTypeArray = vtkTypeInt32Array::SafeDownCast(abstractPrecipitationTypeArray);
    ...
    for (int i = 0; i < expectedPointAmount; i++) {
        ...
        for (type = disallowedPrecipitationTypes.begin(); type != disallowedPrecipitationTypes.end(); ++type) {
...
TEST(TestPrecipitationTypeFilter, TestFilter) {
    ...
    std::unique_ptr<JsonReader> jsonReader = JsonReaderFactory::createReader("res/test-data/precipitation-types.kJson");
    ...
    filter->Update();
    outputDataSet->ShallowCopy(filter->GetOutput());
    testDataSet(outputDataSet, totalNumberOfPoints - (80 * 1), QList<PrecipitationDataPoint::PrecipitationType>() << PrecipitationDataPoint::NONE);
    
    // Additionally, exclude data points with the precipitation type rain
    filter->enableRain(0);
    filter->Update();
    outputDataSet->ShallowCopy(filter->GetOutput());
    testDataSet(outputDataSet, totalNumberOfPoints - (80 * 2), QList<PrecipitationDataPoint::PrecipitationType>() << PrecipitationDataPoint::NONE << PrecipitationDataPoint::RAIN);
    
    // Additionally, exclude data points with the precipitation type snow
    filter->enableSnow(0);
    filter->Update();
    outputDataSet->ShallowCopy(filter->GetOutput());
    testDataSet(outputDataSet, totalNumberOfPoints - (80 * 3), QList<PrecipitationDataPoint::PrecipitationType>() << PrecipitationDataPoint::NONE << PrecipitationDataPoint::RAIN << PrecipitationDataPoint::SNOW);
    
    // Additionally, exclude data points with the precipitation type sleet
    filter->enableSleet(0);
    filter->Update();
    outputDataSet->ShallowCopy(filter->GetOutput());
    testDataSet(outputDataSet, totalNumberOfPoints - (80 * 4), QList<PrecipitationDataPoint::PrecipitationType>() << PrecipitationDataPoint::NONE << PrecipitationDataPoint::RAIN << PrecipitationDataPoint::SNOW << PrecipitationDataPoint::SLEET);
    
    // Additionally, exclude data points with the precipitation type hail, therefore eliminating all data points
    filter->enableHail(0);
    filter->Update();
    outputDataSet->ShallowCopy(filter->GetOutput());
    testDataSet(outputDataSet, 0, QList<PrecipitationDataPoint::PrecipitationType>() << PrecipitationDataPoint::NONE << PrecipitationDataPoint::RAIN << PrecipitationDataPoint::SNOW << PrecipitationDataPoint::SLEET << PrecipitationDataPoint::HAIL);
