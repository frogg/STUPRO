...
class DataUtility:
    ...
    def read_tabbed_file(self, filename):
        """
        Read the contents of a tabbed file and return it as a list of dictionaries using the keys
        specified in the first line of the file.
        """
        with open(filename, 'rb') as tabbed_file:
            return [row for row in csv.DictReader(tabbed_file, delimiter='\t', quotechar='"')]
