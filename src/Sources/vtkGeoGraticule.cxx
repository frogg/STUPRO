...
void vtkGeoGraticule::PrintSelf( ostream& os, vtkIndent indent ) {
	...
	os << indent << "LatitudeBounds:  [ " <<  this->LatitudeBounds[0] << ", " <<
	   this->LatitudeBounds[1] << " [\n";
	os << indent << "LongitudeBounds: [ " << this->LongitudeBounds[0] << ", " <<
	   this->LongitudeBounds[1] << " [\n";
}
...
int vtkGeoGraticule::RequestData( vtkInformation*, vtkInformationVector**,
...
void vtkGeoGraticule::GenerateGraticule( vtkPolyData* output, double latbds[2], double lngbds[2] ) {
	...
	for ( lat = latbds[0]; lat < latbds[1] + latTicIncrement; lat += latTicIncrement, ++ p ) {
		...
		if ( this->GeometryType & vtkGeoGraticule::POLYLINES ) {
			...
			width->InsertNextValue( this->ComputeLineLevel( p, this->LatitudeLevel,
			                        vtkGeoGraticule::LatitudeLevelTics ) );
		}
	}
