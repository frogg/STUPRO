...
public:
	...
	void Render(vtkRenderer* ren, int shape, int style, double margin, double color[3],
	            double opacity) const {
		if (shape == KronosLabelMapper::NONE) {
			return;
		}
		...
		switch (shape) {
		case KronosLabelMapper::ROUNDED_RECT: {
		...
		poly->SetPoints(pts);
		if (style == KronosLabelMapper::OUTLINE) {
			poly->SetLines(cells);
		} else {
			poly->SetPolys(cells);
		}
...
class KronosLabelMapper::Internal {
public:
...
//----------------------------------------------------------------------------
KronosLabelMapper::KronosLabelMapper() {
...
//----------------------------------------------------------------------------
int KronosLabelMapper::FillInputPortInformation(
...
//----------------------------------------------------------------------------
void KronosLabelMapper::RenderOverlay(vtkViewport* viewport,
	...
	for ( ; ! inIter->IsAtEnd(); inIter->Next() ) {
		...
		if ( this->Debug ) {
			vtkDebugMacro("Try: " << inIter->GetLabelId() << " (" << ll[0] << ", " << ll[1] << "  " << ur[0] <<
			              "," << ur[1] << ")");
			if ( labelType == 0 ) {
				if ( this->UseUnicodeStrings ) {
		...
		if ( this->PlaceAllLabels || this->Buckets->PlaceLabel( r ) ) {
			...
			vtkDebugMacro("Placed: " << inIter->GetLabelId() << " (" << ll[0] << ", " << ll[1] << "  " << ur[0]
			              << "," << ur[1] << ") " << labelType);
			placed++;
		}
	}
...
//----------------------------------------------------------------------------
void KronosLabelMapper::PrintSelf( ostream& os, vtkIndent indent ) {
	...
	os << indent << "GeneratePerturbedLabelSpokes: " << (this->GeneratePerturbedLabelSpokes ? "ON" :
	        "OFF" ) << "\n";
